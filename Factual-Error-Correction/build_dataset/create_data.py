...
def read_fairseq_files():
    """Read fairseq documents (source) and summaries (target).
    """
    # data = []
    # dataset_ = load_dataset('ccdv/cnn_dailymail', '3.0.0')
    # for i, (s, t) in enumerate(zip(dataset_['train']['article'], dataset_['train']['highlights'])):
    #     s, t = s.strip(), t.strip()
    #     data.append({
    ...
    data = []
    dataset_ = pd.read_csv('../data_epfl_news/news_data_val_prep.csv')
    dataset_ = dataset_.dropna()
    for i, (s, t) in enumerate(zip(dataset_['document_prep'], dataset_['summary_prep'])):
        s, t = s.strip(), t.strip()
        data.append({
            'id': i, 'text': s, 'summary': t
        })
...
def save_data(args, data, name_suffix):
    output_file = "/mlodata1/blinova/Factual-Error-Correction/data_epfl_news/news-val" + "-" + name_suffix +  ".jsonl"
...
def main(args):
    # load data
    source_docs = read_fairseq_files()
