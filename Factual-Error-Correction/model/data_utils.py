...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    #                         src_ids, tgt_ids, src_attention_mask, src_token_type_ids)
    dataset = TensorDataset(text_ids, text_attention_mask,
                            src_ids, tgt_ids, src_attention_mask)
...
def convert_text_and_summary(examples, tokenizer,
    ...
    for ex_index, example in enumerate(examples):
        ...
        text_ids = text_["input_ids"]
        text_token_type_ids = [1]*len(text_ids)
        # text_ids, text_token_type_ids = text_["input_ids"], text_["token_type_ids"]

        summary_ = tokenzie(tokenizer, summary, 100, False)
        summary_ids = summary_["input_ids"]
        summary_token_type_ids = [1]*len(summary_ids)
