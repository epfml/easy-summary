...
class T5FineTuner_summary(pl.LightningModule):
    ...
    #              *args, **kwargs):
        super(T5FineTuner_summary, self).__init__()
        self.args = args
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.args.custom_loss:
            ...
            lambda_ = 0.2
            loss = complex_score ** 2 + loss + lambda_ *(complex_score - loss)
    ...
    def rouge_loss(self,batch):
        ...
        for i in range(len(batch)):
            pred = generate(batch['source'][i])
            ref = batch['targets'][i]
            scores = scorer.score(ref, pred)
            RougeL_score_f += scores['rougeL'][2]

        RougeL_score_f = RougeL_score_f / len(batch)
        return 1-RougeL_score_f
...
def train_summary(args):
    ...
    model = T5FineTuner_summary(args)
